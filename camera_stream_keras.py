# modified camera-opencv (github) miguelgrinberg code
...
class Camera(BaseCamera):
    video_source = 0
    model_path = 'trained_model/v2/model/model_195.h5'
    ...
    def frames():
        ...
        model = cnn_model_2()
        model.load_weights(Camera.model_path)
        ...
        while True:
            ...
            for (top, right, bottom, left) in face_locations:
                ...
                cv2.putText(frame, prediction, (left, bottom), cv2.FONT_HERSHEY_DUPLEX, 1.5, (0, 255, 0), 1)

            timestamp = datetime.datetime.now()
            frame = cv2.putText(frame, timestamp.strftime("%A %d %B %Y %I:%M:%S%p"), (10, 15), cv2.FONT_HERSHEY_DUPLEX, 0.6, (255, 0, 0), 1)
